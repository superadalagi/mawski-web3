...
export async function runInit(options) {
    ...
    if (detected.aliasedInstall) {
        fail(`This project installs v5 under the alias "${detected.aliasedInstall}". init is for new apps only.`);
        info(`  ${color.dim('fix:')} ${LEGACY_COEXISTENCE_HINT}`);
        return 1;
    }
    if (detected.legacyInstall) {
        fail(`This project already has ${PACKAGE_NAME}@${detected.legacyInstall} (pre-v5). init is for new apps only.`);
        info(`  ${color.dim('fix:')} ${LEGACY_COEXISTENCE_HINT}`);
        return 1;
    }
    ...
    fail(`${failed.length} check${failed.length === 1 ? '' : 's'} failed. Run \`npx ${PACKAGE_NAME}@alpha doctor\` for details.`);
    return 1;
}
