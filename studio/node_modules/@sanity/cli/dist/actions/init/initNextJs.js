...
export async function initNextJs({ datasetName, detectedFramework, envFilename, mcpConfigured, options, output, projectId, trace, workDir }) {
    const { install, nextjsAppendEnv, nextjsEmbedStudio, packageManager, template, typescript, unattended } = options;
    ...
    if (install) {
        ...
        output.log(`Skipped dependency install. Run ${getAddCommand(chosen, [
            ...packages,
            NEXT_SANITY_PACKAGE
        ])} to add them.`);
    }
