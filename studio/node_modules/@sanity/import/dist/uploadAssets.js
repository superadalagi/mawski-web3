...
export async function uploadAssets(assets, options) {
    ...
    const ensureMethod = options.allowFailingAssets
        ? (assetKey, i) => ensureAssetExists(assetKey, i).catch((err) => err)
        : ensureAssetExists;
...
async function ensureAssetWithRetries(options, progress, assetKey, i) {
    ...
    const { buffer, sha1hash } = await retryOnFailure(() => downloadAsset(url, i, options.assetFetch)).catch((err) => {
        progress();
...
function downloadAsset(url, i, assetFetch) {
...
async function ensureAsset(asset, options, i) {
    ...
    if (!replaceAssets) {
        ...
        const assetDocId = await getAssetDocumentIdForHash(client, type, sha1hash, 0, suffixTag(tag, 'asset.get-id'), options.assetFetch);
        if (assetDocId) {
...
async function getAssetDocumentIdForHash(client, type, sha1hash, attemptNum, tag, assetFetch) {
...
function setAssetReferences(assetRefMap, assetIds, options) {
    ...
    for (const task of patchTasks) {
        ...
        if (overallSize + task.tasks.length > ASSET_PATCH_BATCH_TASK_SIZE ||
