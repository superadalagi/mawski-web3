...
export async function importBatches(batches, options) {
    ...
    const result = { count: 0, importedIds: [] };
...
function importBatch(options, progress, batch) {
    ...
    return retryOnFailure(async () => {
        ...
        const docsTransaction = docs.length > 0
            ...
            : Promise.resolve({ count: 0, importedIds: [] });
        const releaseResults = releaseDocs.length > 0
            ? await pMap(releaseDocs, (doc) => {
                const actionParams = {
            ...
            }, { concurrency: RELEASE_IMPORT_CONCURRENCY, stopOnError: false }).catch((err) => {
