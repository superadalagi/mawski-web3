...
import { EVENT_DOCUMENT_CREATE, EVENT_DOCUMENT_DELETE, EVENT_DOCUMENT_UPDATE, EVENT_MEDIA_LIBRARY_ASSET_CREATE, EVENT_MEDIA_LIBRARY_ASSET_DELETE, EVENT_MEDIA_LIBRARY_ASSET_UPDATE, EVENT_PUBSUB, EVENT_SCHEDULED, EVENT_SYNC_TAG_INVALIDATE, FUNCTION_TYPES, LABEL_DOCUMENT_CREATE, LABEL_DOCUMENT_DELETE, LABEL_DOCUMENT_UPDATE, LABEL_MEDIA_LIBRARY_ASSET_CREATE, LABEL_MEDIA_LIBRARY_ASSET_DELETE, LABEL_MEDIA_LIBRARY_ASSET_UPDATE, LABEL_PUBSUB, LABEL_SCHEDULED, LABEL_SYNC_TAG_INVALIDATE, MAP_EVENT_TO_FUNCTION_TYPE, SANITY_FUNCTION_DOCUMENT, SANITY_FUNCTION_MEDIA_LIBRARY_ASSET, SANITY_FUNCTION_PUBSUB, SANITY_FUNCTION_SCHEDULED, SANITY_FUNCTION_SYNC_TAG_INVALIDATE, } from '../../constants.js';
...
export const generateFunctionBlueprintResourceTemplate = (fnName, eventNames) => {
    ...
    switch (functionType) {
        ...
        case SANITY_FUNCTION_PUBSUB:
            definer = `definePubSubFunction({name: '${fnName}'}), // ← add this line`;
            break;
...
export async function functionAddCore(options) {
    ...
    const blueprintFileDir = dirname(blueprintFilePath);
    const warnings = getWarnings({ dir: blueprintFileDir });
    ...
    try {
        ...
        if (eventSources.size > 1) {
            throw new Error('Invalid function type. Cannot mix document-*, media-library-asset-*, pub-sub, sync-tag-* and/or scheduled-* types.');
        }
        let addHelpers;
        let installCommand;
        if (!['ts', 'js'].includes(fnLang)) {
            ...
            installCommand = addHelpers ? await promptForInstallCommand(blueprintFileDir) : null;
        }
        if (installCommand)
...
async function promptForFunctionName() {
    const functionName = await input({
        ...
        validate: (input) => validateFunctionName(input) ||
            'Invalid function name. Must be 3+ characters, no special characters, no spaces',
    });
    return functionName;
}
async function promptForFunctionType() {
    ...
    const functionTypes = await checkbox({
        ...
        choices: [
            ...
            { name: LABEL_PUBSUB, value: EVENT_PUBSUB },
        ...
        validate(choices) {
            ...
            // same grouping as the check in functionAddCore, so the prompt cannot drift from it
            const eventSources = new Set(choices.map((c) => String(c.value).substring(0, String(c.value).lastIndexOf('-'))));
            if (eventSources.size > 1) {
                return 'You cannot mix Document, Media Library Asset, PubSub, Scheduled, and Sync Tag Invalidate events together in one Function';
            }
            return true;
        },
    });
...
async function promptForInstallCommand(dir) {
    // hydrate only supports these three; a detected bun falls back to npm
    const detected = detectPackageManager(dir);
    const preferred = INSTALLER_OPTIONS.find((option) => option === detected) ?? 'npm';
    const command = await select({
        message: 'How to install the @sanity/functions helpers:',
        choices: [
            ...INSTALLER_OPTIONS.map((option) => ({ name: option, value: option })),
