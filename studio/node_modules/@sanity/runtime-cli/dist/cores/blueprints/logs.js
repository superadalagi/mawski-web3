...
export async function blueprintLogsCore(options) {
    ...
    try {
        ...
        if (watch) {
            const { ok, logs, error } = await getLogs({ stackId, limit: WATCH_PREFETCH_LIMIT }, auth, fetchFn);
            if (!ok) {
        ...
        const { ok, logs, error, hasMore } = await getLogs({ stackId, limit, before, after: since }, auth, fetchFn);
        if (!ok) {
