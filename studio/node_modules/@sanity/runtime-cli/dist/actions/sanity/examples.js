...
async function extractExampleFromArchive({ archiveStream, exampleType, exampleName, }) {
            ...
            if (type === 'file') {
                ...
                stream.on('data', (chunk) => {
                    if (!(chunk instanceof Uint8Array)) {
                        stream.destroy(new TypeError('Expected archive entry data to be a Uint8Array'));
                        return;
                    }
                    chunks.push(chunk);
                });
    ...
    // not ideal to cast as unknown as Writable...
    // but tar-stream uses streamx, which interoperates with Node streams
