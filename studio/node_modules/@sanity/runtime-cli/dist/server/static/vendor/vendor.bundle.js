...
var ViewState = class {
	...
	measure() {
		...
		if (!this.inView && !this.scrollTarget && !inWindow$1(view.dom)) return 0;
...
//#endregion
//#region node_modules/@enhance/store/index.mjs
var _state = {};
var dirtyProps = [];
var listeners = [];
...
var timeout;
var handler = { set: function(obj, prop, value) {
	if (prop === "initialize" || prop === "subscribe" || prop === "unsubscribe") return false;
	if (obj[prop] !== value) {
		obj[prop] = value;
...
function Store(initialState) {
	if (initialState) initialize(initialState);
	return store;
}
function merge(o, n) {
	for (let prop in n) o[prop] = n[prop];
}
/**
...
function initialize(initialState) {
	if (initialState) merge(_state, initialState);
}
/**
...
function notify() {
	listeners.forEach((l) => {
		...
		if (Object.keys(payload).length) fn(payload);
	});
	dirtyProps.length = 0;
}
//#endregion
...
var toLocaleString = (number, locale, options) => {
	...
	else if (locale === true) result = number.toLocaleString(void 0, options);
	else if (options !== void 0) result = number.toLocaleString("en-US", {
		...options,
		useGrouping: false
	});
	return result;
...
function prettyBytes(number, options) {
	...
	const base = options.binary ? 1024 : 1e3;
	let exponent = 0;
	if (number >= 1) exponent = Math.min(Math.floor(options.binary ? log(number) / Math.log(base) : log10(number) / 3), UNITS.length - 1);
	...
	return applyFixedWidth(prefix + toLocaleString(number, options.locale, localeOptions) + separator + UNITS[exponent], options.fixedWidth);
}
//#endregion
