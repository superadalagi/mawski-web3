...
class FiltersComponent extends ApiBaseElement {
  ...
  renderFilters = () => {
    ...
    const pubsubFunction = this.api.store.selectedFunctionType === this.SANITY_FUNCTION_PUBSUB
  ...
  buildFilters = (docFunction, mediaFunction, scheduleFunction, stiFunction, pubsubFunction) => {
    return `
    <legend class="config-label">Client Options</legend>
    <div id="dynamic-dropdowns" class="flex gap-2">
      ${
        docFunction || scheduleFunction || stiFunction || pubsubFunction
          ? `<select-dropdown
        label="Project"
        store-key="projects"
