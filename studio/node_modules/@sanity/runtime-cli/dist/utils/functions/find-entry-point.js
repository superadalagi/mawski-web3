 ...
 */
export async function findFunctionEntryPoint(functionPath, displayName) {
    let stats;
    try {
        stats = await stat(functionPath);
    }
    catch (err) {
        throw new Error(`Source path not found or inaccessible: ${functionPath}`, { cause: err });
    }
    if (stats.isFile()) {
    ...
    if (stats.isDirectory()) {
        ...
        try {
            const pkgJsonPath = join(functionPath, 'package.json');
            const pkgJsonContent = await readFile(pkgJsonPath, 'utf8');
            const pkgJson = JSON.parse(pkgJsonContent);
            if (pkgJson.main) {
                const mainPath = resolve(functionPath, pkgJson.main);
        ...
        // 2. Check index.ts
        const indexTs = join(functionPath, 'index.ts');
        if (await fileExists(indexTs)) {
            return indexTs;
        }
        // 3. Check index.js
        const indexJs = join(functionPath, 'index.js');
        if (await fileExists(indexJs)) {
            return indexJs;
        }
        const nameHint = displayName ? ` for function "${displayName}"` : '';
        throw new Error(`Could not determine entry file${nameHint} in directory: ${functionPath}. Looked for package.json#main, index.ts, index.js.`);
    }
    // Should not happen if stat succeeded, but defensively handle
    throw new Error(`Source path is neither a file nor a directory: ${functionPath}`);
}
/**
